...
import { defaultSchedule, hm, normalizeSchedule, workStats } from './worktime'

const STORAGE_KEY = 'island.time.v2' // 继续沿用，靠字段补全做兼容，不丢老数据

// 岛内可显示的四种功能
export const MODES = [
  // name 只在岛内二级导航（CardCarousel 身份行）显示，所以用短标签；
  // 设置面板里的显示名来自 src/settings/apps.js，不受影响。
  { id: 'clock', name: '时间', icon: 'clock', accent: '#0A84FF' },
  { id: 'countdown', name: '计时', icon: 'timer', accent: '#30D158' },
...
function load() {
  ...
  // 早期版本有「专注关联待办」，该功能已移除；顺手清掉老数据里的残留字段
  delete s.focus.taskId
  delete s.focus.taskTitle
...
// ---------- 倒计时：结束后的「再来一次 / 完成」 ----------
// 放在模块级，这样通知条上的动作按钮可以直接调用，不必先 useTime()
function cdAgain() {
  const cd = state.cd
