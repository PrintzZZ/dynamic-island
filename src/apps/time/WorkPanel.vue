<template>
  <div class="work-panel">
    ...
    <div class="head">
      <button
        class="back"
        :title="view === 'settings' ? '返回' : '返回模式选择'"
        @click="view === 'settings' ? (view = 'detail') : emit('close')"
      >
...
import { now, useTime } from './useTime'

const { state, work, setWorkSchedule } = useTime()
const w = work

// 详情页的返回键要退回二级导航；设置页的返回键退回详情页
