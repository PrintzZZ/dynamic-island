...
<table>
<tr>
<td width="50%" align="center"><b>紧凑态</b><br/><sub>屏幕顶部的小胶囊，日常时间只留居中时间 + 右侧圆环</sub></td>
<td width="50%" align="center"><b>展开态</b><br/><sub>悬停展开成卡片，顶部是应用切换、下面是对应面板</sub></td>
</tr>
<tr>
...
**独立设置窗口**（980×700，自绘标题栏）—— 常规 / 外观 / 行为 / 应用 / 关于五页，改完立即生效、不需要重启：
...
<tr>
<td><img src="docs/images/settings-general.png" alt="设置 · 常规" /></td>
<td><img src="docs/images/settings-appearance.png" alt="设置 · 外观" /></td>
<td><img src="docs/images/settings-apps.png" alt="设置 · 应用" /></td>
</tr>
</table>

> 截图由 `docs/images/` 里的真实构建产物离屏渲染生成，不是手绘稿；生成方式见 [docs/images/README.md](docs/images/README.md)。
...
- 🔤 **字体**：西文/数字用 **Inter**、中文用 **PingFang SC 苹方**（六个字重 100/200/300/400/500/600，700 与 800 复用 Semibold，避免合成假粗）；字体文件放 `src/assets/fonts/`（见该目录 README）即可自动生效。未放文件时自动回退到 MiSans → Segoe UI + 微软雅黑，构建不受影响。⚠️ 苹方是 Apple 专有字体，**仅限本地使用，请勿提交到公开仓库或随包分发**。
- 🎨 **内联 SVG 图标**：全部图标由 `Icon.vue` 以 SVG 渲染（`currentColor` 着色），跨平台渲染一致、不失真，无字体符号依赖。
- ⚙️ **设置面板**：**独立的设置窗口**（980×700，自绘标题栏，拖动不频闪），不进胶囊。左侧导航（常规 / 外观 / 行为 / 应用 / 关于）+ 右侧分组卡片，视觉是 macOS 系统设置 + Windows Fluent + 灵动岛的融合。可配开机自启、启动位置与显示器、悬停/收起/Esc、顶部吸附、主题、动画（完整/简洁/关闭）、视觉效果、通知时长与分项开关、音量与提示音，以及**模块与每个小功能的单独启停**（时间四个模式、待办/便签、剪贴板/常用语/材料箱、网速都能各自开关）与详细设置（剪贴板 / 时间 / 材料箱）。入口在**托盘菜单**和**岛内右键菜单**。设置统一存在 `userData/settings.json`（主进程），两个窗口经 IPC **实时双向同步、改完立即生效**（不用重启）；「恢复默认设置」只重置偏好，不碰便签 / 待办 / 时间数据 / 剪贴板历史 / 材料箱任务。
- 🧩 **可扩展应用框架**：统一注册表，每个应用只需一个 manifest 即可接入。
- 📝 **便签**：增删改、颜色标签、本地持久化、紧凑态显示最新摘要。
- ✅ **待办**：iOS 圆形勾选框、进度条、快捷添加、清除已完成、本地持久化。
- ⏲️ **时间（时间中心）**：日常时间 / 倒计时 / 提醒 / 专注集成在一个应用里，顶部「岛内显示」分段控件决定紧凑胶囊显示哪一个。胶囊内为**模式图标 + 两行文案 + 环形进度**；**日常时间例外 —— 岛内只留居中的时间与右侧圆环**，不显示图标与日期。
  ...
  - 🔔 **提醒**：**一次性 / 每日**两种类型，一次性支持今天 / 明天 / 指定日期 / **相对时间**（30分钟后…）；催办策略可选 `关闭 / 1次 / 3次 / 直到确认` × 间隔 `30秒 / 1分钟 / 5分钟`，**两种类型都按策略催，直到点「知道了」**。列表与新建表单都是 **iOS 分组样式**（发丝线分行、大号等宽时间、悬停删除），「下一提醒」胶囊卡同时也是新建入口 `＋`。
...
- 📋 **剪贴板**：自动记录复制历史（去重、上限可配、写入用户数据目录），支持搜索、点击复制、清空；**点击复制有三重反馈**（行高亮 + 行内「已复制」胶囊 + 底部浮层提示 + 轻提示音）；**识别到网址时条目右侧提供「一键跳转」**，同时岛会形变到通知态提醒并可直接打开链接。
...
│   │   └── useSettings.js      # 设置镜像（hydrate 一次 + 本地同步读 + 乐观写回）
│   ├── settings/               # 独立设置窗口（单独入口，多页输入）
│   │   ├── SettingsApp.vue     # 标题栏 + 左导航 + 中内容 + 右功能栏
│   │   ├── SideRail.vue        # 右栏：预览 / 音效 / 通知 / 恢复默认
│   │   ├── components/         # Card / Row / Toggle / Segmented / Select / Slider / Check
│   │   └── sections/           # 常规 / 外观 / 行为 / 应用 / 关于
│   ├── utils/
│   │   └── sound.js            # Web Audio 音效引擎（音量与分项开关来自设置）
│   ├── assets/fonts/           # 本地字体（Inter / PingFang SC / MiSans，见其 README）
│   ├── apps/
│   │   ├── registry.js         # 应用注册表
│   │   ├── index.js            # 应用注册入口
│   │   ├── notes/              # 便签应用
